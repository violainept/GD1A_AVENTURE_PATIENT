...
public class PlayerAttack : MonoBehaviour
{
    ...
    private IEnumerator IEAttack()
    {
        ...
        if (CurrentWeapon.WeaponType == WeaponType.Magic) // if the player weapon is magic, do a magic attack
        {
            ...
            playerAnimations.SetMagicAttackAnimation(true);
            yield return new WaitForSeconds(0.5f);
            playerAnimations.SetMagicAttackAnimation(false);
        }
        else // if the player weapon isn't magic, do a melee attack
        {
            MeleeAttack();

            playerAnimations.SetMeleeAttackAnimation(true);
            yield return new WaitForSeconds(0.5f);
            playerAnimations.SetMeleeAttackAnimation(false);
        }
    }
